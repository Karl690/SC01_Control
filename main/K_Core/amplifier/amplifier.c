...
uint16_t AmpBaseAddres = 0;
uint16_t ProcessTimer = 0;
uint16_t TotalPower = 0;
uint16_t Rf_Power_Ch1 = 0;
uint16_t Rf_Power_Ch2 = 0;
uint16_t Rf_Power_Ch3 = 0;
uint16_t Rf_Power_Ch4 = 0;
uint16_t Rf_Power_Ch5 = 0;
uint16_t Rf_Power_Ch6 = 0;
uint16_t ArrayPluggedIn = 0;
...
uint8_t amplifier_channel_index = 0;

uint8_t amplifier_logging[MAX_CHANNELS];
void amplifier_init()
{
...
void amplifier_build_status_string()
{
	...
	if (!amplifier_logging[amplifier_channel_index]) return; 
	sprintf(amplifier_temp_string,
		"Q%d %d %d %d %d %d %d %d %d %d\n",
		CurrentAmpStatus.channel,
		CurrentAmpStatus.Process_Timer,
		CurrentAmpStatus.TotalPower,
		CurrentAmpStatus.Pwr1,
		CurrentAmpStatus.Pwr2,
		CurrentAmpStatus.Pwr3,
		CurrentAmpStatus.Pwr4,
		CurrentAmpStatus.Pwr5,
		CurrentAmpStatus.Pwr6,
		CurrentAmpStatus.array_plugged_in);
...
//	ui_simple_add_log(amplifier_temp_string, UI_SEND_COLOR);
//	communication_add_string_to_serial_buffer(&ComUart2.TxBuffer, amplifier_temp_string);
//	if (amplifier_logging[amplifier_channel_index] == 0xff)
//		{ amplifier_logging[amplifier_channel_index] = 0; }
//	amplifier_channel_index++;
//	if (amplifier_channel_index >= MAX_CHANNELS) amplifier_channel_index = 0;
//if logging flag was set to 255 it means one time report only
}
void amplifier_set_logging(uint8_t address, uint8_t status)
{
	if (address == 0)
	{	
