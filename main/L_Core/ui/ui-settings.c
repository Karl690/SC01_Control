...
lv_obj_t* ui_settings_mode_page;
...
void ui_settings_event_submenu_cb(lv_event_t* e)
{
	...
	switch (type)
	{
	case SETTINGS_SUBMENU_MODE:
		settings_active_page = ui_settings_mode_page;
		break;
...
/*
 *sets the pins used for tx, rx, and default baud rate
 *needed because we debug on differnt hardware platforms with different pinouts
 **/
void ui_settings_serial_load_event_cb(lv_event_t* e)
{
	UI_SETTINGS_SERIAL* ui_serial = settings_active_page == ui_settings_serial_page1 ? &ui_settings.ui_serial1 : &ui_settings.ui_serial2;
	uint8_t* data = (uint8_t*)lv_event_get_user_data(e);
	if (data == 0)  //preset for K or L  
	{
		//preset for K core
		if (settings_active_page == ui_settings_serial_page1)
		{
			//serial page 1 , comm to 407 amplifier module 0
		...
		{
			// serial port 2  used for secs and simple serial , default is 9600
			...
			lv_dropdown_set_selected(ui_serial->ui_baud, get_index_from_baud(9600));
		}
	...
	{
		//presets for L core  used for debugging for lvana
		if (settings_active_page == ui_settings_serial_page1)
		{
			lv_dropdown_set_selected(ui_serial->ui_tx_pin, get_index_from_value(GPIO_NUM_21));
			lv_dropdown_set_selected(ui_serial->ui_rx_pin, get_index_from_value(GPIO_NUM_14));
...
void ui_settings_event_save_cb(lv_event_t* e)
{
	...
	systemconfig.mode.option = lv_dropdown_get_selected(ui_settings.ui_mode.option);
...
void ui_settigs_event_serial_button_cb(lv_event_t* e)
{
	...
	case 0:
		if(run_mode != RUN_BLE_CLIENT)
			communication_tx_commandline(MegComPort, KEYBOARD_SHFIT_ADDRESS_STRING); 
		break;
	case 1:
		if (run_mode != RUN_BLE_CLIENT)
			communication_tx_commandline(MegComPort, KEYBOARD_RESET_ADDRESS_STRING); 
		break;
	}
}

void ui_settings_mode_page_init()
{
	ui_settings_mode_page = lv_obj_create(ui_settings_screen);
	lv_obj_set_size(ui_settings_mode_page, 375, 225);//480-105
	lv_obj_set_pos(ui_settings_mode_page, 102, 55); 
	lv_obj_set_style_pad_all(ui_settings_mode_page, 10, LV_PART_MAIN);
	lv_obj_t* obj = ui_create_label(ui_settings_mode_page, "MODE", &lv_font_montserrat_20);
	lv_obj_set_align(obj, LV_ALIGN_TOP_MID);
	
	char szModes[100] = "Disabled\nHeating\nCooling";
	
	uint16_t x = 0, y = 30;	
	obj = ui_create_label(ui_settings_mode_page, "Mode: ", &lv_font_montserrat_14);
	lv_obj_set_pos(obj, 0, y);
	obj = lv_dropdown_create(ui_settings_mode_page);
	lv_dropdown_set_options(obj, szModes);
	...
	lv_dropdown_set_selected(obj, systemconfig.mode.option);
	ui_settings.ui_mode.option = obj;
...
void ui_settings_sdcard_page_init()
{
	...
	lv_obj_set_size(ui_settings_sdcard_page, 375, 225); //480-105
	lv_obj_set_pos(ui_settings_sdcard_page, 102, 55); 
...
{
	UI_SETTINGS_SERIAL* ui_serial = index == 0 ? &ui_settings.ui_serial1 : &ui_settings.ui_serial2;
...
void ui_settings_screen_init()
{
	...
	lv_obj_t* obj = ui_create_button(submenu, "MODE", LV_PCT(90), h, 3, &lv_font_montserrat_14, ui_settings_event_submenu_cb, (void*)SETTINGS_SUBMENU_MODE);
	lv_obj_set_user_data(obj, (void*)SETTINGS_SUBMENU_MODE);
	lv_obj_set_style_bg_color(obj, lv_color_hex(UI_BUTTON_ACTIVE_BG_COLOR), LV_PART_MAIN);
	lv_obj_set_pos(obj, 0, y);
	settings_active_menu = obj;

	obj = ui_create_button(submenu, "Bluetooth", LV_PCT(90), h, 3, &lv_font_montserrat_14, ui_settings_event_submenu_cb, (void*)SETTINGS_SUBMENU_BLUETOOTH);
	...
	obj = ui_create_button(submenu, "SD CARD", LV_PCT(90), h, 3, &lv_font_montserrat_14, ui_settings_event_submenu_cb, (void*)SETTINGS_SUBMENU_SDCARD);
...
void ui_settings_update_configuratiion()
{
	...
	lv_dropdown_set_selected(ui_settings.ui_mode.option, systemconfig.mode.option);
}
